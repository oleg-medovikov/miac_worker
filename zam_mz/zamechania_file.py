...
def zamechania_file():
    ...
    DF = covid_sql(SQL)
    DF = DF.loc[~DF["Тип организации"].isnull()]
    ...
    ws = wb["amb_dist"]
    PART = DF.loc[
        (DF["Тип организации"] == "Амбулаторная")
        & ~(
            DF["Принадлежность"].isin(
                ["комитет здравоохранения", "частные", "федеральные"]
            )
        )
    ].copy()
    del PART["Тип организации"]
    ...
    PART = DF.loc[
        (DF["Тип организации"] == "Амбулаторная")
        & (DF["Принадлежность"].isin(["комитет здравоохранения"]))
    ].copy()
    del PART["Тип организации"]
    del PART["Принадлежность"]
    rows = dataframe_to_rows(PART, index=False, header=False)
    ...
    PART = DF.loc[
        (DF["Тип организации"] == "Амбулаторная")
        & (DF["Принадлежность"].isin(["федеральные"]))
    ].copy()
    del PART["Тип организации"]
    del PART["Принадлежность"]
    rows = dataframe_to_rows(PART, index=False, header=False)
    ...
    PART = DF.loc[
        (DF["Тип организации"] == "Амбулаторная")
        & (DF["Принадлежность"].isin(["частные"]))
    ].copy()
    del PART["Тип организации"]
    del PART["Принадлежность"]
    rows = dataframe_to_rows(PART, index=False, header=False)
    ...
    ws = wb["stat_dist"]
    PART = DF.loc[
        (DF["Тип организации"] == "Стационарная")
        & ~(
            DF["Принадлежность"].isin(
                ["комитет здравоохранения", "частные", "федеральные"]
            )
