select f.УНРЗ,f.ФИО,f.[Дата рождения],f.[Медицинская организация], isnull(c.[Примечания],'') as [Примечания]
  from(
   select dbo.get_Gid(idPatient) as gid,[УНРЗ],[ФИО],[Дата рождения],[Медицинская организация]
      from robo.v_FedReg
      where [СНИЛС] = 'Не идентифицирован' and [Исход заболевания] not in ('Смерть')
	  ) f
