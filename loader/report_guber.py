...
header_vp = [
    ...
    "nameMO",
    "indicators",
    "vp_Received_Count_All_SPb",
    "vp_Received_Count_All_LO",
    "vp_Received_Count_toDay_Spb",
    "vp_Received_Count_toDay_LO",
    "vp_Discharged_Count_All_SPb",
    "vp_Discharged_Count_All_LO",
    "vp_Discharged_Count_toDay_Spb",
    "vp_Discharged_Count_toDay_LO",
    "vp_Died_Count_All_SPb",
    "vp_Died_Count_All_LO",
    "vp_Died_Count_toDay_Spb",
    "vp_Died_Count_toDay_LO",
    "vp_Hospital_Count_All",
    "vp_Hospital_Count_Spb",
    "vp_Hospital_Count_LO",
...
header_cv = [
    ...
    "nameMO",
    "indicators",
    "cv_Diagnosis_Count_All_SPb",
    "cv_Diagnosis_Count_All_LO",
    "cv_Diagnosis_Count_toDay_Spb",
    "cv_Diagnosis_Count_toDay_LO",
    "cv_Discharged_Count_All_SPb",
    "cv_Discharged_Count_All_LO",
    "cv_Discharged_Count_toDay_Spb",
    "cv_Discharged_Count_toDay_LO",
    "cv_Died_Count_All_SPb",
    "cv_Died_Count_All_LO",
    "cv_Died_Count_toDay_Spb",
    "cv_Died_Count_toDay_LO",
    "cv_Hospital_Count_All",
    "cv_Hospital_Count_Spb",
    "cv_Hospital_Count_LO",
...
header_ivl = [
    ...
    "nameMO",
    "ivl_Invaz_Count_All",
    "ivl_Invaz_Count_Busy",
    "ivl_Invaz_Count_Free_All",
    "ivl_Invaz_Count_Faulty",
    "ivl_NeInvaz_Count_All",
    "ivl_NeInvaz_Count_Busy",
    "ivl_NeInvaz_Count_Free_All",
    "ivl_NeInvaz_Count_Faulty",
    "ivl_Pacient_Count_All",
    "ivl_Pacient_Count_Covid",
]
...
def load_sheet(file, sheetName, ColumsName, startRows, header_):
    df = pd.read_excel(
        file, sheet_name=sheetName, header=None, usecols=ColumsName, skiprows=startRows
    )
...
def report_guber():
    ...
    for FILE in glob.glob(DIRECTORY + "/из_почты/[!~$]*.xls*"):
        load_file(
            FILE, "Cвод_ОРВИ_и_Пневм", "A:S", 4, header_vp, "ReportGubernator_Pnevm"
        )
    ...
    if check_data_tab("mon_vp.v_DebtorsReportGubernator"):
        df = covid_sql(
            "SELECT [Наименование МО] as 'должники!' FROM mon_vp.v_DebtorsReportGubernator"
        )
    ...
    new_file1 = (
        "temp/09_стационары для Справки Губернатора_"
        + datetime.datetime.now().strftime("%d.%m.%Y_%H_%M")
        + ".xlsx"
    )
    new_file2 = (
        "temp/09_стационары для Справки Губернатора_"
    ...
    df_covid = covid_sql(
        "SELECT * FROM [mon_vp].[v_GrandReport_Guber_Covid] order by idRows"
    )
    df1_covid = (
        df_covid.drop("Установлены диагнозы: вчера", 1)
    ...
    df_pnev = covid_sql(
        "SELECT * FROM [mon_vp].[v_GrandReport_Guber_Pnev] order by idRows"
    )
    df1_pnev = (
        df_pnev.drop("Установлены диагнозы: вчера", 1)
        .drop("Установлены диагнозы: должно быть", 1)
        .drop("Установлены диагнозы: фактически", 1)
        .drop("на стационарном лечении: вчерашние данные", 1)
        .drop("на стационарном лечении: должно быть", 1)
    ...
    df_ivl = covid_sql(
        "SELECT * FROM [mon_vp].[v_GrandReport_Guber_Ivl] order by idRows"
    )

    df_bunk = covid_sql(
        "SELECT * FROM [mon_vp].[v_GrandReport_Guber_Bunk] order by idRows"
    )

    df_sys = covid_sql("SELECT * FROM mon_vp.v_GrandReport")

    df1_sys = (
        df_sys.loc[df_sys.typeMO == 1]
        .sort_values(["numSort"])
        .drop("typeMO", 1)
        .drop("numSort", 1)
    )
    df2_sys = (
        df_sys.loc[df_sys.typeMO == 2]
        .sort_values(["numSort"])
        .drop("typeMO", 1)
        .drop("numSort", 1)
    )
    ...
    ws = wb1["Свод"]
    ws["Q2"] = (datetime.datetime.now() + datetime.timedelta(days=1)).strftime(
        "%d.%m.%Y"
    )
