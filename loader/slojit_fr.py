...
def read_file(FILE):
    df = pd.read_excel(FILE, header=1, usecols=NAMES, engine="xlrd", skipfooter=1)
    return df
...
WHERE [date_rows] < GETDATE()"""
    ...
    count_vizd_new = df[
        df["Исход заболевания"].isin(["Выздоровление"]) & df["Диагноз"].isin(["U07.1"])
    ].shape[0]
    ...
    df["Дата исхода заболевания"] = pd.to_datetime(
        df["Дата исхода заболевания"], format="%d.%m.%Y", errors="ignore"
    )
...
"""
    ...
    DICT_REPLACE = {False: True, True: False}
    ...
    try:
        count_deti_death_old = covid_sql(
            f"""
        SELECT [value_count] FROM [robo].[values]
            where id = (select max(id)
                from [robo].[values]
        ...
        raise my_except("нет цифры всего детей умерло от COVID вчера")
...
"""
    ...
    count_deti_death_else = df.loc[COVID_D_E & AGE_18 & SCHOOL & COVID & DEATH].shape[0]
    ...
    try:
        ...
        my_except("нет цифры всего заболело от ковид вчера")
    ...
    try:
        ...
        my_except("Нет числа всего школьников выздоровело вчера")
    ...
    try:
        ...
        my_except("Нет числа всего школьников умерло вчера")
    print("test 2")
...
"""
    ...
    df["Диагноз установлен"] = pd.to_datetime(
        df["Диагноз установлен"], format="%d.%m.%Y", errors="ignore"
    )
    df["Дата исхода заболевания"] = pd.to_datetime(
        df["Дата исхода заболевания"], format="%d.%m.%Y", errors="ignore"
    )
