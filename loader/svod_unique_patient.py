...
def svod_unique_patient(DATE_GLOBAL):
    ...
    # Записываем файл
    FILE = Dir.get('covi_list') + '/Автосвод ' \
            + datetime.strptime(DATE_GLOBAL, '%d.%m.%Y').strftime('%Y-%m-%d') +'.csv'
    ...
    # Выводим ответ
    mess = 'Свод уникальных пациентов за дату ' \
            + DATE_GLOBAL + ' сделан!' \
            + '\nНайдено дублей: ' + str(len(DUBLI)) \
            + '\nНовых пациентов: ' + str(len(RPN)) \
